...
def show_login_window(root):
    ...
    login_window.protocol("WM_DELETE_WINDOW", on_close)  # Gestion du bouton de fermeture 'X'


    # Contenu de la fenêtre
    tk.Label(login_window, text="Choisissez un mode de connexion", font=('Helvetica', 12)).pack(pady=10)

    # Mode Admin
    tk.Button(login_window, text="Admin", font=('Helvetica', 12),
              command=lambda: admin_login(login_window, root)).pack(pady=5)

    # Mode Utilisateur
    tk.Button(login_window, text="Utilisateur", font=('Helvetica', 12),
...
def display_emplacements(tree, record_count_textbox, filter_text=""):
    for row in tree.get_children():
        tree.delete(row)  # Effacer les anciennes données
